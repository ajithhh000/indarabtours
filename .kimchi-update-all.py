...
HOME_NAV = '''<nav id="navbar" class="fixed top-0 left-0 right-0 z-50 bg-white transition-shadow duration-300">
    <div class="container-main flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 md:h-20">
      ...
      <ul class="hidden md:flex items-center gap-1 font-cta text-sm font-semibold uppercase tracking-wide">
            <li><a href="" class="px-3 py-2 text-primary border-b-2 border-primary transition-colors duration-200">Home</a></li>
            ...
            <li><a href="about/" class="px-3 py-2 text-text-secondary hover:text-primary border-b-2 border-transparent transition-colors duration-200">About</a></li>
    ...
    <div id="mobile-menu" class="md:hidden hidden bg-white border-t border-gray-100 shadow-lg">
      <ul class="flex flex-col font-cta text-sm font-semibold uppercase tracking-wide">
            <li><a href="" class="block px-6 py-3.5 text-primary bg-surface/50 border-l-4 border-primary">Home</a></li>
            ...
            <li><a href="about/" class="block px-6 py-3.5 text-text-secondary hover:bg-surface hover:text-primary border-l-4 border-transparent transition-colors">About</a></li>
...
def make_nav(active):
    ...
    for key, href, label in pages:
        if key == active:
            desktop_items.append(f'<li><a href="{href}" class="px-3 py-2 text-primary border-b-2 border-primary transition-colors duration-200">{label}</a></li>')
            mobile_items.append(f'<li><a href="{href}" class="block px-6 py-3.5 text-primary bg-surface/50 border-l-4 border-primary">{label}</a></li>')
        else:
            desktop_items.append(f'<li><a href="{href}" class="px-3 py-2 text-text-secondary hover:text-primary border-b-2 border-transparent transition-colors duration-200">{label}</a></li>')
            mobile_items.append(f'<li><a href="{href}" class="block px-6 py-3.5 text-text-secondary hover:bg-surface hover:text-primary border-l-4 border-transparent transition-colors">{label}</a></li>')
    desktop = '\n            '.join(desktop_items)
    mobile = '\n            '.join(mobile_items)
    return f'''<nav id="navbar" class="fixed top-0 left-0 right-0 z-50 bg-white transition-shadow duration-300">
    <div class="container-main flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16 md:h-20">
      <!-- Logo -->
      <a href="../" class="flex items-center gap-2.5 flex-shrink-0" aria-label="IND ARAB TOURS AND TRAVELS Home">
        <img src="../public/logo-mark.png" alt="IND ARAB TOURS AND TRAVELS logo" width="40" height="40" class="h-10 w-auto">
...
FOOTER_ROOT = '''<footer class="bg-primary text-white">
...
FOOTER_SUB = FOOTER_ROOT.replace('href="/', 'href="../').replace('src="/', 'src="../')
...
def replace_google_fonts(text):
    # Remove all Google Fonts related tags and replace with single set
    text = re.sub(
    ...
    # Insert HOME_FONTS after manifest or before stylesheet
    text = re.sub(
...
def standardize_subpage(path, page_title, page_subtitle, bg_image, active_key, extra_cleanup=None):
    text = Path(path).read_text(encoding='utf-8')

    text = replace_google_fonts(text)
    if extra_cleanup:
    ...
    # Normalize section-padding: ensure sections with py-16 md:py-20/24 also have px-4 sm:px-6 lg:px-8
    # We leave existing section classes as-is since most already include padding.

    Path(path).write_text(text, encoding='utf-8')
    print(f"Updated {path}")

def cleanup_contact(text):
    # Remove Tailwind CDN
    text = re.sub(
...
def cleanup_resources(text):
    # Remove inline font style block
    text = re.sub(
        r'<style>\s*body\s*{\s*font-family:\s*[\'"]Source Sans 3[\'"],\s*[\'"]Source Sans Pro[\'"],\s*sans-serif;\s*}\s*h1,\s*h2,\s*h3,\s*h4,\s*h5,\s*h6\s*{\s*font-family:\s*[\'"]Playfair Display[\'"],\s*serif;\s*}\s*\.font-cta\s*{\s*font-family:\s*[\'"]Montserrat[\'"],\s*sans-serif;\s*}\s*</style>\s*',
        '',
        text,
        flags=re.DOTALL
    )
...
if __name__ == '__main__':
    ...
    standardize_subpage(
        base / 'destinations/index.html',
        'Destinations',
        'Handpicked international destinations for travelers from the UAE',
    ...
    standardize_subpage(
        ...
        'Travel Resources',
        'Everything you need to plan your perfect international trip from the UAE',
        'https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=1600&h=900&fit=crop',
        'resources',
        extra_cleanup=cleanup_resources
    )
    standardize_subpage(
        base / 'contact/index.html',
        'Contact Us',
        'Get in touch with our travel experts for a personalized consultation',
        'https://images.unsplash.com/photo-1423666639041-f56000c27a9a?w=1600&h=900&fit=crop',
        'contact',
        extra_cleanup=cleanup_contact
    )
